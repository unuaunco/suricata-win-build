...
msys2_shell.cmd -defterm -no-start -mingw64 -here -c '''yes | pacman -S --noconfirm --needed base-devel
mingw-w64-x86_64-toolchain subversion'''

msys2_shell.cmd -defterm -no-start -mingw64 -here -c "pacman -S 
...
mingw64/mingw-w64-x86_64-luajit-git wget jansson  jansson-devel libpcre pcre \
    ZZZZZpcre-devel gcc  gcc-libs  make autoconf autogen automake git libyaml \
...
msys2_shell.cmd -defterm -no-start -mingw64 -here -c "git clone -b suricata-6.0.2 https://github.com/OISF/suricata.git && cd suricata && \
